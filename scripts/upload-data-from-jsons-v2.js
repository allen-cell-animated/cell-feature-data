...
const features = require('../data-2-0/measured-features')
...
const writeFeatureKeys = () => {
    return Promise.all(features.map((ele) => ref.collection('measured-features-names').doc(ele.key).set(ele)))
        
}

const formatCellLineDefs = () => (
    fsPromises.readFile('./dataset-2-0/cell-line-def.json')
            .then((data) => JSON.parse(data))
...
const writeFovMapping = () => {
    ...
    return formatCellLineDefs()
        .then(cellLineDefs => {
            ...
            fsPromises.readFile('./dataset-2-0/cell-feature-analysis.json')
                .then((data) => JSON.parse(data))
                .then((json) => {
